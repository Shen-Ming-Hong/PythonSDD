...
| 編號 | 操作 | 預期結果 | 狀態 |
|------|------|----------|------|
| M-001 | 使用 360px 寬直向視窗開啟首頁 | 無水平捲動；遊戲畫布維持 4:3；左右與中央按鈕都在畫面內 | 待人工驗收 |
| M-002 | 按下「發球」 | 球開始運動，生命與分數文字可見 | 待人工驗收 |
| M-003 | 按住左／右按鈕至少 1 秒，再放開 | 底板向對應方向移動；放開後停止，不會卡住 | 待人工驗收 |
| M-004 | 按住方向按鈕後滑出、取消觸控或切換分頁再回來 | 輸入被清除，底板不會自行持續移動 | 待人工驗收 |
| M-005 | 觸發勝利或 Game Over 後按「重玩」 | 磚塊、生命、分數與球回到初始狀態 | 待人工驗收 |
| M-006 | 使用低高度橫向手機視窗 | 上下間距縮小；遊戲區與控制仍可使用 | 待人工驗收 |
| M-007 | 回到桌面寬度使用 Space、方向鍵、A／D | 原有鍵盤操作仍可使用，規則沒有變化 | 待人工驗收 |
